...
def login():
    if is_rate_limited(request.remote_addr):
        return json.dumps(
            {
                "status": "error",
                "errors": ["Too many requests! Try again in a few seconds."],
            }
        )
...
def check():
    ...
    try:
        ...
        if auth != AUTH_PASSWORD:
            return json.dumps(
                {"status": "error", "errors": ["Incorrect auth! Login and try again."]}
            )
    except ValueError:
    ...
    if errors:
        ...
        return json.dumps(
            {
                "status": "success",
                "secret": "true" if secret.upper() == PASSWORDS[level - 1] else "false",
            }
        )
...
def ai():
    ...
    if not errors:
        try:
            ...
            err = (
                "There was an error getting a reply from the chatbot, please try again."
            )
